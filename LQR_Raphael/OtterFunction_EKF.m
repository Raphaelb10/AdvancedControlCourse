...
%% Define symbolic variables inputs
x = sym("x",[12,1],'real'); %current state
u = sym('u',[2,1],'real'); % input from motors 
%% No ocean current considered in the model
V_c = 0;
beta_c = 0;
%% No load
...
Thrust = zeros(2,1);
% for i = 1:1:2
%     if u(i) > n_max              % saturation, physical limits
%        u(i) = n_max; 
%     elseif n(i) < n_min
%        u(i) = n_min; 
%    end
%     
%    if u(i) > 0                          
%      Thrust(i) = k_pos * u(i)*abs(u(i));    % positive thrust (N) 
%    else
...
% M\.. do not work with symbolic variables, must use inv(M)
% Time derivative of the state vector - numerical integration; see ExOtter.m  
xdot = [ inv(M) * ( tau + tau_damp + tau_crossflow - C * nu_r - G * eta - g_0)
         velocities ];  
